...
dependencias() {
  soft="sudo bsdmainutils zip unzip ufw curl python python3 python3-pip openssl screen cron iptables lsof pv boxes nano at mlocate gawk grep bc jq curl npm nodejs socat netcat netcat-traditional cowsay figlet lolcat apache2"
...
install_oficial() {
  ...
  cd /etc
  wget https://github.com/Jaok04/TvRc-Mda/raw/refs/heads/main/Vps-Mx/VPS-MX_8.5.tar.xz >/dev/null 2>&1
...
install_mod() {
  ...
  mkdir /etc/VPS-MX >/dev/null 2>&1
  cd /etc
  wget https://github.com/Jaok04/TvRc-Mda/raw/refs/heads/main/Vps-Mx/VPS-MX_8.6.tar.xz >/dev/null 2>&1
  tar -xf VPS-MX.tar.xz >/dev/null 2>&1
  chmod +x VPS-MX.tar.xz >/dev/null 2>&1
  rm -rf VPS-MX.tar.xz
  cd
